...
cv2.imwrite("01-gray.jpg", gray)

cblur = cv2.GaussianBlur(im,(1,1),1000)
cv2.imwrite("02-cblur.jpg", cblur)

blur = cv2.GaussianBlur(gray,(1,1),1000)
cv2.imwrite("02-blur.jpg", blur)

channels = cv2.split(cblur)
i = 0
cannys = []
for channel in channels:
    cv2.imwrite("04-05-%d.jpg" % i, channel)
    canny = cv2.Canny(channel, 10, 100)
    cannys.append(canny)
    cv2.imwrite("04-10-%d.jpg" % i, canny)
    i += 1


prev = cannys[0]
for canny in cannys[1:]:
    prev = cv2.bitwise_xor(prev, canny)
cv2.imwrite("05-noise.jpg", prev)




flag, thresh = cv2.threshold(blur, 140, 255, cv2.THRESH_BINARY)
cv2.imwrite("10-thresh.jpg", thresh)
...
cv2.imwrite("11-contours.jpg", im)
