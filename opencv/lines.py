...
def get_orientation(l):
    """ Get the angle of the given line. The returned angle is between -Pi/2
        and Pi/2.
    """
    x = 1.0 * l[0] - l[2]
    y = 1.0 * l[1] - l[3]
...
def is_aligned(a, b, threshold = math.pi/180):
    """ Return true if lines a and b are aligned.
    """
    oa = get_orientation(a)
...
def is_parallel(a, b, threshold = math.pi/180):
    """ Return True is the two given lines are parallel
    """
    oa = get_orientation(a)
    ob = get_orientation(b)
    if abs(ob - oa) <= threshold or abs(ob + oa) <= threshold:
        return True
    return False

def get_points_from_line(l):
    """ Return the two points of the given line
    """
    a = np.array([l[0], l[1]])
    b = np.array([l[2], l[3]])
    return a,b
...
def get_contours(img):
    """ Extract paper contours from the given image
    """
    ...
    flag, thresh = cv2.threshold(img, 140, 255, cv2.THRESH_BINARY)
    edges = cv2.Canny(thresh, 80, 120)
    return edges



orig = cv2.imread("amundi.jpg")

edges = get_contours(orig)
